...
def obtener_mi_perfil():
    try:
        ...
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        # Nueva consulta con múltiples JOINs
        query = """
            SELECT 
                u.*, 
                dg.nacionalidad_id, dg.fecha_nacimiento, dg.estado_civil_id, dg.sexo_id, dg.discapacidad, dg.pais_id, dg.provincia_id, dg.ciudad_id,
                ...
                da.titulo_homologado, da.unidad_educativa, da.tipo_unidad_educativa_id, da.calificacion, da.cuadro_honor, da.titulo_tercer_nivel, da.titulo_cuarto_nivel, da.fecha_registro_nacional
            FROM usuarios u
